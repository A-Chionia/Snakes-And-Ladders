  ...
  const startGame = async () => {
    ...
    return (
        <div className="sidePanel">
          ...
          {!correctPlayers && !gameStarted && <p>This game requires between 1 and 4 players.</p>}

          {fluidWorks === true && !gameStarted && <div className="verticalButtons">
          <p>Welcome to the treacherousss land of many, many snakes and some ladders!</p>
          <button onClick={buttonClicked}>
          {playText}
          </button>
          {activeUsers.length > 0 && <div className="playertitle"><p>Players</p></div>}
          {activeUsers.length > 0 && <div className="players"> {activeUsers.map((name, index) => (
            typeof name === 'string' ? <div key={index}>{name}</div> : null
            ))}</div> }

          <button className="ready" onClick={startGame}>Everyone ready?</button>
          </div>}
